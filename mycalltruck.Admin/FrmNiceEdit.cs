...
{
    public partial class FrmNiceEdit : Form
    {
     ...
     //   string linkId = "edubillsys";
        ...
        public FrmNiceEdit(int SalesId,string _BillNo)
        {
            InitializeComponent();
            FSalesId = SalesId;
            OriginalbillNo = _BillNo;
        }
        ...
        {
            salesManageTableAdapter1.Fill(salesDataSet.SalesManage);

            var _Q = salesDataSet.SalesManage.Where(c => c.SalesId == FSalesId).ToArray();

            if (_Q.Any())
            {
                var _QSelected = _Q.First();
                dtp_RequestDate.Value = _QSelected.RequestDate;
                txt_SangHo.Text = _QSelected.SangHo;
                txt_BizNo.Text = _QSelected.BizNo;
                txt_CEO.Text = _QSelected.Ceo;
                txt_ContRactName.Text = _QSelected.ContRactName;
                txt_Uptae.Text = _QSelected.Uptae;
                txt_Upjong.Text = _QSelected.Upjong;
                txt_Email.Text = _QSelected.Email;
                txt_State.Text = _QSelected.AddressState;
                txt_City.Text = _QSelected.AddressCity;
                txt_AddrDetail.Text = _QSelected.AddressDetail;
                txt_Zip.Text = _QSelected.ZipCode;

                txt_MobileNo.Text = _QSelected.MobileNo;

                if (_QSelected.Taxgubun == 0)
                {
                    rdb_Tax0.Checked = true;
                }
                else if (_QSelected.Taxgubun == 1)
                {
               ...
               if(_QSelected.TypeCode == "위수탁")
                ...
                salesManageDetailTableAdapter.Fill(salesDataSet.SalesManageDetail);
                var _DQ = salesDataSet.SalesManageDetail.Where(c => c.SalesId == FSalesId).ToArray();

                if(_DQ.Any())
                {
                    
                    txt_Item1.Text = _DQ.First().itemName1;
                    txt_Num1.Text = _DQ.First().qty1;

                    if (rdb_Tax1.Checked)
                    {
                        txt_Price1.Text = _DQ.First().unitCost1.ToString();
                    }
                    else
                    {
                        txt_UnitPrice1.Text = _DQ.First().unitCost1.ToString();
                    }
                    txt_Price1.Text = _DQ.First().supplyCost1.ToString("N0");
                    txt_VAT1.Text = _DQ.First().tax1.ToString("N0");

                    if (!String.IsNullOrEmpty(_DQ.First().itemName2))
                    {
                        txt_Item2.Text = _DQ.First().itemName2;
                        txt_Num2.Text = _DQ.First().qty2;
                        if (rdb_Tax1.Checked)
                        {
                            txt_Price2.Text = _DQ.First().unitCost2.ToString();
                        }
                        else
                        {
                            txt_UnitPrice2.Text = _DQ.First().unitCost2.ToString();
                        }
                        txt_Price2.Text = _DQ.First().supplyCost2.ToString("N0");
                        txt_VAT2.Text = _DQ.First().tax2.ToString("N0");
                    }


                    if (!String.IsNullOrEmpty(_DQ.First().itemName3))
                    {
                        txt_Item3.Text = _DQ.First().itemName3;
                        txt_Num3.Text = _DQ.First().qty3;
                        if (rdb_Tax1.Checked)
                        {
                            txt_Price3.Text = _DQ.First().unitCost3.ToString();
                        }
                        else
                        {
                            txt_UnitPrice3.Text = _DQ.First().unitCost3.ToString();
                        }
                        txt_Price3.Text = _DQ.First().supplyCost3.ToString("N0");
                        txt_VAT3.Text = _DQ.First().tax3.ToString("N0");
                    }


                    if (!String.IsNullOrEmpty(_DQ.First().itemName4))
                    {
                        txt_Item4.Text = _DQ.First().itemName4;
                        txt_Num4.Text = _DQ.First().qty4;
                        if (rdb_Tax1.Checked)
                        {
                            txt_Price4.Text = _DQ.First().unitCost4.ToString();
                        }
                        else
                        {
                            txt_UnitPrice4.Text = _DQ.First().unitCost4.ToString();
                        }
                        txt_Price4.Text = _DQ.First().supplyCost4.ToString("N0");
                        txt_VAT4.Text = _DQ.First().tax4.ToString("N0");
                    }


                }
                txt_Amount.Text = (_DQ.First().supplyCost1 + _DQ.First().supplyCost2 + _DQ.First().supplyCost3 + _DQ.First().supplyCost4).ToString("N0");
            }
            ...
            if (_UpdateDB() > 0)
            {
               ...
               if (rdb_Tax2.Checked)
                ...
                else
                {
                    ...
                    {
                        Dialog_EtaxManageNiceREdit d = new Dialog_EtaxManageNiceREdit(_rdopurposeType, _staxtype, "Direct", FSalesId, OriginalbillNo);
                        d.Sales = Datas.ToArray();
