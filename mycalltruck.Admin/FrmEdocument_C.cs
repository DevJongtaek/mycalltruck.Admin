...
{
    public partial class FrmEdocument_C : Form
    {
        // private File _b;
        //public static EnumerableRowCollection<TradeDataSet.TradesRow> Trade { get; set; }
        string CarYear = "";
        string MobileNo = "";
        int _ITrade = 0;
        string _PdfFileName = "";

        public FrmEdocument_C(int itrade,string PdfFileName)
        {
            InitializeComponent();
            _ITrade = itrade;
            _PdfFileName = PdfFileName;
            tradesTableAdapter1.Fill(this.tradeDataSet1.Trades);
            var  Trade = tradeDataSet1.Trades.Where(c => c.TradeId == _ITrade).FirstOrDefault();
        ...
        {
            var Trade = tradeDataSet1.Trades.Where(c => c.TradeId == _ITrade).FirstOrDefault();
            var url = "http://m.cardpay.kr/Cardpay-Data/Tax_Images/" + Trade.DriverId.ToString() + "/" + _PdfFileName;

            lblKeyword.Text = _PdfFileName.Replace("signed_", "").Replace(".pdf", "");
            ...
            webClient.DownloadFile(url, di +@"\" + _PdfFileName);

            


            pdfViewer1.Document?.Dispose();
            pdfViewer1.Document = OpenDocument(di + @"\" + _PdfFileName);
