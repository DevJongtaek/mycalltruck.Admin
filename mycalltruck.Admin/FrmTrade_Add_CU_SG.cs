...
{
    public partial class FrmTrade_Add_CU_SG : Form
    {
        Int64 iPrice = 0;
        DESCrypt m_crypt = null;
        bool AllowTaxBool = false;
        public FrmTrade_Add_CU_SG()
        {
            InitializeComponent();
        ...
        private void FrmTrade_Add_Load(object sender, EventArgs e)
        {
            ...
            mDriverRepository.Select_Self(baseDataSet.Drivers);
        }
        ...
        private bool _AddClient()
        {
            try
            {
                ...
                if (Selected == null)
                    return false;
                var Driver = baseDataSet.Drivers[0];
                using (SqlConnection cn = new SqlConnection(Properties.Settings.Default.TruckConnectionString))
                {
                    ...
                    _DriverCommand.Parameters.AddWithValue("@DriverId", Driver.DriverId);
                    var PayBankName = "";
                    var PayBankCode = "";
                    var PayAccountNo = "";
                    var PayInputName = "";
                    ...
                    cmd.CommandText =
                        "INSERT Trades (RequestDate, BeginDate, EndDate, Item, Price, VAT, Amount, PayState, PayDate, PayBankName, PayBankCode, PayAccountNo, PayInputName, DriverId, ClientId, UseTax, HasAcc, AllowAcc, CustomerId, BizNo, Name)" +
                        "Values (@RequestDate, @BeginDate, @EndDate, @Item, @Price, @VAT, @Amount, 2" +
                        ", @PayDate, @PayBankName, @PayBankCode, @PayAccountNo, @PayInputName, @DriverId, @ClientId" +
                        ", 1, 1, 0, @CustomerId, @BizNo, @Name)";
                    ...
                    cmd.Parameters.AddWithValue("@BizNo", Selected.BizNo);
                    cmd.Parameters.AddWithValue("@Name", Selected.SangHo);
                    cmd.ExecuteNonQuery();
                    cn.Close();
                }
            }
        ...
        private void btn_InfoSearch_Click(object sender, EventArgs e)
        {
            ...
            customersBindingSource.Filter = $"SangHo LIKE '%{txt_InfoSearch.Text}%' OR BizNo LIKE '%{txt_InfoSearch.Text}%' OR PhoneNo LIKE '%{txt_InfoSearch.Text}%'";
