...
{
    public partial class FrmMN0303_CARGOFPIS_Add2 : Form
    {
        string SFpisId = string.Empty;
        string SGoodsUnit = string.Empty;
        public FrmMN0303_CARGOFPIS_Add2(string FpisId,string GoodsUnit)
        {
        ...
        private void _AddClient()
        {
            ...
            row.TRU_TRANS_TYPE = cmb_TRU_TRANS_TYPE.SelectedValue.ToString();
            ...
            row.TRU_CONT_ITEM = cmb_TRU_CONT_ITEM.SelectedValue.ToString();
            row.TRU_GOODS_FORM = cmb_TRU_GOODS_FORM.SelectedValue.ToString();
            row.TRU_GOODS_CNT = txt_TRU_GOODS_CNT.Text;

            row.TRU_GOODS_UNIT = SGoodsUnit;

            row.TRU_START_ADDR = txtStartZip.Text;
            row.TRU_START_ADDR1 = txtStartAddr.Text;

            row.TRU_END_ADDR = txtEndZip.Text;
            row.TRU_END_ADDR1 = txtEndAddr.Text;
        ...
        {
            Regex emailregex = new Regex(@"[0-9]");
            Boolean ismatch = emailregex.IsMatch(txt_TRU_GOODS_CNT.Text);
            if (!ismatch)
            {
                MessageBox.Show("숫자만 입력해 주세요.");
