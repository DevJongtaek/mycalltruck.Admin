...
{
    public partial class FrmMN0209_CUSTOMER_Customer : Form
    {
        int GridIndex = 0;
        ...
        public FrmMN0209_CUSTOMER_Customer()
        {
            InitializeComponent();
            ...
            if (!String.IsNullOrEmpty(txt_LoginId.Text) && !String.IsNullOrEmpty(txt_Password.Text))
            {
                using (SqlConnection _Connection = new SqlConnection(Properties.Settings.Default.TruckConnectionString))
                {
                    ...
                    _Command.CommandText = "SELECT COUNT(*) FROM " +
                        ...
                        " union " +
                        " Select LoginId, customerid From CustomerUsers) as a  " +
        ...
        private void newDGV2Binding(int CustomerId)
        {
            ...
            using (SqlConnection _Connection = new SqlConnection(Properties.Settings.Default.TruckConnectionString))
            {
                ...
                _Command.CommandText =
                      ...
                      @"select idx, AddTeam, AddName, AddPhoneNo, ClientId, CustomerId, CreateTime,Remark from CustomerAddPhone where CustomerId = @CustomerId
        ...
        private void btnPUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrWhiteSpace(txtPIdx.Text))
            {
                ...
                using (SqlConnection _Connection = new SqlConnection(Properties.Settings.Default.TruckConnectionString))
                {
                    ...
                    _Command.CommandText = "Update CustomerAddPhone SET AddTeam = @AddTeam" +
                        ...
                        " ,Remark =  @Remark" +
                        " WHERE idx = @idx ";
                    _Command.Parameters.AddWithValue("@AddTeam", txtAddTeam.Text);
            ...
            else
            {
                ...
                using (SqlConnection _Connection = new SqlConnection(Properties.Settings.Default.TruckConnectionString))
                {
                    _Connection.Open();
                    SqlCommand _Command = _Connection.CreateCommand();
                    _Command.CommandText = "INSERT INTO CustomerAddPhone (AddTeam, AddName, AddPhoneNo, ClientId, CustomerId, CreateTime,Remark) " +
                        " VALUES ( @AddTeam, @AddName, @AddPhoneNo, @ClientId, @CustomerId, getdate(),@Remark)";
                    _Command.Parameters.AddWithValue("@AddTeam", txtAddTeam.Text);
                    _Command.Parameters.AddWithValue("@AddName", txtAddName.Text);
                    _Command.Parameters.AddWithValue("@AddPhoneNo", txtAddPhoneNo.Text);
        ...
        private void btnPDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("정말 전화번호 추가정보를 삭제하겠습니까?", "차세로", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                using (SqlConnection _Connection = new SqlConnection(Properties.Settings.Default.TruckConnectionString))
                {
                    _Connection.Open();
