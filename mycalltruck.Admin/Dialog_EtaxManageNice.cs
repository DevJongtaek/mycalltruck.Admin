...
{
    public partial class Dialog_EtaxManageNice : Form
    {
       ...
       // string linkId = "edubillsys";
        ...
        public Dialog_EtaxManageNice(string _rdopurposeType1,string _taxType,string _Gubun)
        {
            InitializeComponent();
            clientsTableAdapter.FillByClientId(this.cmDataSet.Clients, LocalUser.Instance.LogInInformation.ClientId);
         ...
         //   var _Admininfo = baseDataSet.AdminInfoes.ToArray().FirstOrDefault();
            ...
            Thread t = new Thread(new ThreadStart(() =>
            {
                ...
                foreach (var item in AcceptSales)
                {
                    ...
                    {
                        Result = dTIServiceClass.makeAndPublishSign(linkcd, _Clients.frnNo, _Clients.userid, _Clients.passwd, certPw, dtiXml, "Y", "N", "");
                    }
                            ...
                            try
                            {
                                ...
                                try
                                {
                                    using (SqlConnection cn = new SqlConnection(Properties.Settings.Default.TruckConnectionString))
                                    {
                                        ...
                                        cmd.CommandText =
                                           "UPDATE SalesManage SET IssueDate = @IssueDate ,RequestDate = @RequestDate, Issue = 1  , IssueState = '발행', HasAcc = 1,billNo = @billNo,IssueLoginId = @IssueLoginId, IssueGubun = @IssueGubun,purposeType = @purposeType  WHERE    SalesId =" + item.SalesId + " ";
                                        ...
                                        cmd.Parameters.AddWithValue("@TypeCode", "정발행");
                                        cmd.ExecuteNonQuery();
