...
{
    public partial class Dialog_EtaxManageNiceREdit : Form
    {
       ...
       // string linkId = "edubillsys";
        ...
        public Dialog_EtaxManageNiceREdit()
        {
            InitializeComponent();
        ...
        public Dialog_EtaxManageNiceREdit(string _rdopurposeType1, string _taxType, string _Gubun, int _FSalesId = 0, string _BillNo = "")
        {
            InitializeComponent();
            clientsTableAdapter.FillByClientId(this.cmDataSet.Clients, LocalUser.Instance.LogInInformation.ClientId);
        ...
        private void Step5Etax()
        {
            ...
            Thread t = new Thread(new ThreadStart(() =>
            {
                ...
                foreach (var item in AcceptSales)
                {
                    ...
                    $"      <OriginalIssueID>{OriginalbillNo}</OriginalIssueID> <!-- 당초 전자(세금)계산서 승인번호 -->\r\n" +
                    $"  </TaxInvoiceDocument>\r\n" +
                        ...
                        if (retVal == "0")
                        {
                            ...
                            try
                            {
                                ...
                                try
                                {
                                    ...
                                    try
                                    {
                                        #region 기존세금계산서 마이너스발행
                                        if (FSalesId != 0)
                                        {
                                            salesManageTableAdapter.Fill(salesDataSet.SalesManage);
                                            var _Query = salesDataSet.SalesManage.Where(c => c.SalesId == FSalesId).FirstOrDefault();
                                            using (SqlConnection cn = new SqlConnection(Properties.Settings.Default.TruckConnectionString))
                                            {
                                                cn.Open();
                                                SqlCommand cmd = cn.CreateCommand();
                                                ...
                                                cmd.CommandText =
                                                   "UPDATE SalesManage SET IssueDate = getdate()  , IssueState = '취소' ,billNo = @billNo,IssueLoginId = @IssueLoginId, IssueGubun = @IssueGubun  WHERE    SalesId =" + FSalesId.ToString() + " ";
                                                cmd.Parameters.AddWithValue("@billNo", _Query.billNo);
                                                cmd.Parameters.AddWithValue("@IssueLoginId", LocalUser.Instance.LogInInformation.LoginId);
                                                cmd.Parameters.AddWithValue("@IssueGubun", "차세로");
                                                cmd.ExecuteNonQuery();
