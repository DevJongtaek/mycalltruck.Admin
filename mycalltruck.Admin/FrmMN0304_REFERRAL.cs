...
{
    public partial class FrmMN0304_REFERRAL : Form
    {
        ...
        {
            // TODO: 이 코드는 데이터를 'nSTATSDataSet.NSTATS1CUSTOMERETC' 테이블에 로드합니다. 필요 시 이 코드를 이동하거나 제거할 수 있습니다.
            this.nSTATS1CUSTOMERETCTableAdapter.Fill(this.nSTATSDataSet.NSTATS1CUSTOMERETC);
            //NSTAT1();
...
//               SalesManage ON Orders.SalesManageId = SalesManage.SalesId";
            ...
            return @"SELECT  Orders.CreateTime, ISNULL(Orders.Customer, '') AS CustomerName, Orders.DriverCarNo, Orders.Driver, Orders.DriverPhoneNo, ISNULL(Orders.StartName, '') AS StartName, 
               ...
               Orders.OrderLine, Orders.OrderProduct, Orders.OrderTurn, Orders.OrderWidth, Orders.OrderLength, Orders.OrderHeight,AlterTradePrice,NTradePrice,NAlterTradePrice,NPrice
FROM     Orders INNER JOIN
               StaticOptions AS CarSize ON Orders.CarSize = CarSize.Value AND CarSize.Div = 'ETCCarSize' INNER JOIN
               StaticOptions AS CarType ON Orders.CarType = CarType.Value AND CarType.Div = 'ETCCarType' INNER JOIN
               StaticOptions AS PayLocation ON Orders.PayLocation = PayLocation.Value AND PayLocation.Div = 'PayLocation' LEFT JOIN
               Trades ON Orders.TradeId = Trades.TradeId LEFT OUTER JOIN
               Customers ON Orders.ReferralId = Customers.CustomerId AND Customers.BizGubun = 4 INNER JOIN
               Clients ON Orders.ClientId = Clients.ClientId INNER JOIN
        ...
        {
            nSTATSDataSet.NSTATS1CUSTOMERETC.Clear();
            ...
            using (SqlConnection _Connection = new SqlConnection(Properties.Settings.Default.TruckConnectionString))
            {
                ...
                using (SqlCommand _Command = _Connection.CreateCommand())
                {
                   ...
                   // WhereStringList.Add(string.Format("Orders.CustomerId  = '{0}'", LocalUser.Instance.LogInInformation.CustomerId));
                    ...
                    WhereStringList.Add(string.Format("Orders.ReferralId  = '{0}'", LocalUser.Instance.LogInInformation.CustomerId));


                    //WhereStringList.Add("Orders.ReferralId = {} ");
                    ...
                    {
                        nSTATSDataSet.NSTATS1CUSTOMERETC.Load(_Reader);
                        ...
                        if (grid1.RowCount > 0)
                        {
                            ...
                            nSTATS1CUSTOMERETCBindingSource_CurrentChanged(null, null);
            ...
            var Query = nSTATSDataSet.NSTATS1CUSTOMERETC.ToArray();

            if (Query.Any())
            {
                lblTradePrice.Text = Query.Sum(c => c.TradePrice).ToString("N0");
              
                lblAlterPrice.Text = Query.Sum(c => c.AlterTradePrice).ToString("N0");
                
                lblTotal.Text = (Query.Sum(c => c.TradePrice) + Query.Sum(c => c.AlterTradePrice)).ToString("N0");

                lblNTradePrice.Text = Query.Sum(c => c.NTradePrice).ToString("N0");

                lblNAlterPrice.Text = Query.Sum(c => c.NAlterTradePrice).ToString("N0");

                lblNPrice.Text = (Query.Sum(c => c.NTradePrice) + Query.Sum(c => c.NAlterTradePrice)).ToString("N0");
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            f.NSTATS1(dtp_Sdate.Text, dtp_Edate.Text, jogun, nSTATS1CUSTOMERETCBindingSource);
        ...
        private void grid1_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            var Selected = ((DataRowView)grid1.Rows[e.RowIndex].DataBoundItem).Row as NSTATSDataSet.NSTATS1CUSTOMERETCRow;
            ...
            {
                e.Value = (Selected.TradePrice + Selected.AlterTradePrice).ToString("N0");
            }

            else if (e.ColumnIndex == NPriceColumns.Index)
            {
                e.Value = (Selected.NTradePrice + Selected.NAlterTradePrice).ToString("N0");
            }
            ...
            else if (e.ColumnIndex == ColumnCustomerTeam.Index)
            {
                ...
                else
                {
                    ...
                    var _Q = customerUserDataSet.CustomerTeams.Where(c => c.ClientId == LocalUser.Instance.LogInInformation.ClientId  && c.CustomerTeamId == Selected.CustomerTeam).ToArray();
        ...
        private void nSTATS1CUSTOMERETCBindingSource_CurrentChanged(object sender, EventArgs e)
        {
            if (nSTATS1CUSTOMERETCBindingSource.Current == null)
            {
                return;
            }
            else
            {
                var Selected = ((DataRowView)nSTATS1CUSTOMERETCBindingSource.Current).Row as NSTATSDataSet.NSTATS1CUSTOMERETCRow;

                GridIndex = nSTATS1CUSTOMERETCBindingSource.Position;
                _SOrderId = Selected.OrderId;
            }
        }
