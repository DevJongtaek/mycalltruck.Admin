...
{
    public partial class UCNSTATS6 : UserControl
    {
        ...
        private String GetSelectCommand()
        {
            return @"SELECT  ReferralAccount.idx, ReferralAccount.PayDate, ReferralAccount.CustomerId,CASE WHEN ReferralAccount.DriverId  = 0 THEN  Customers.SangHo ELSE Drivers.CarYear END as SangHo, ReferralAccount.OutAmount, ReferralAccount.ClientId, ReferralAccount.OrderSdate, 
               ReferralAccount.OrderEdate, ReferralAccount.Amount,ReferralAccount.ReferralCount
FROM     ReferralAccount left JOIN
        ...
        private void cmbsChange()
        {
            ...
            var cmbCustomerMIdDataSource2 = nSTATSDataSet.NSTATS6.Where(c => c.ClientId == LocalUser.Instance.LogInInformation.ClientId && c.PayDate >= _Sdate && c.PayDate < _Edate).Select(c => new { c.SangHo, c.CustomerId }).OrderBy(c => c.CustomerId).ToArray();
            ...
            if (cmbCustomerMIdDataSource2.Any())
            {
                ...
                foreach (var item in cmbCustomerMIdDataSource2.GroupBy(c => c.CustomerId))
                {


                    DCustomerI.Add((int)item.First().CustomerId, item.First().SangHo);
        ...
        private void 정산내역취소ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (ModelDataGrid.SelectedCells.Count > 0)
            {
                nSTATS6BindingSource.EndEdit();
                if (nSTATS6BindingSource.Current != null)
                {
                    ...
                    if (((DataRowView)nSTATS6BindingSource.Current).Row is NSTATSDataSet.NSTATS6Row Selected)
                    {
                        ...
                        using (SqlConnection cn = new SqlConnection(Properties.Settings.Default.TruckConnectionString))
                        {
                            ...
                            if (MessageBox.Show($"상호   : {Selected.SangHo}\r\n기간   : {Selected.OrderSdate.ToString("d") + " - " + Selected.OrderEdate.ToString("d")}\r\n건수   : {Selected.ReferralCount} 건\r\n금액   : {Selected.Amount.ToString("N0")} 원\r\n\r\n 정말 취소 하시겠습니까?", "수수료 확인", MessageBoxButtons.YesNo) == DialogResult.Yes)
                            {
                                try
                                {
                                    ...
                                    cmd.CommandText = "Update Orders SET ReferralAccountYN = 'N',ReferralAccountId = 0 WHERE ReferralAccountId = @ReferralAccountId ";

                                    cmd.Parameters.AddWithValue("ReferralAccountId", Selected.idx);
                                    ...
                                    cmd.CommandText = "DELETE  ReferralAccount WHERE idx = @idx";
                                    cmd.Parameters.AddWithValue("@idx", Selected.idx);
                                    cmd.ExecuteNonQuery();
