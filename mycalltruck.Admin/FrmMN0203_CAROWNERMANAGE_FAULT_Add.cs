...
{
    public partial class FrmMN0203_CAROWNERMANAGE_FAULT_Add : Form
    {
        DESCrypt m_crypt = null;
        bool ValidateStart = false;
        int DriverId = 0;
        public FrmMN0203_CAROWNERMANAGE_FAULT_Add()
        {
            InitializeComponent();
        ...
        private void _UpdateDB(Action Done)
        {
            ...
            string _PayBankName = cmb_PayBankName.Text;
            ...
            if (cmb_PayBankName.SelectedIndex !=0 && _PayBankName!="은행선택" && !String.IsNullOrEmpty(SPayAccountNo) && !String.IsNullOrEmpty(SInputName))
            {
            ...
            try
            {
                ...
                //mDriverRepository.CreateDriver(SBiz_NO, SName, SCeo, SUptae, SUpjong, SCeoBirth, SZip, SState, SCity, SStreet, SMobileNo, _PayBankCode,
                //    BankName, SPayAccountNo, SInputName, SCarNo, _CarType, _CarSize, SCarInfo, DealerId,0,0);
                // var BankName = Filter.Bank.BankList.Where(c => c.Value == _PayBankCode).ToArray().First().Text;

           
                mDriverRepository.CreateDriver(SBiz_NO, SName, SCeo, SUptae, SUpjong, SCeoBirth, SZip, SState, SCity, SStreet, SMobileNo, _PayBankCode,
                    _PayBankName, SPayAccountNo, Account_Name, SCarNo, _CarType, _CarSize, SCarInfo, DealerId, 0, 0);
            }
            catch (Exception ex)
            {
                MessageBox.Show("데이터베이스 작업 중 문제가 발생하였습니다. 잠시 후 다시 시도해주시기 바랍니다.", Text, MessageBoxButtons.OK, MessageBoxIcon.Stop);
        ...
        private void btn_DriverExcel_Click(object sender, EventArgs e)
        {
            ...
            if (_DriverType == 1)
            {
                fileString = "차량정보_일괄등록_" + DateTime.Now.ToString("yyyyMMdd") + ".xlsx";
                ieExcel = Properties.Resources.DriverUpload;
            }
            else
            {
                fileString = "차량정보_일괄등록(표준)_" + DateTime.Now.ToString("yyyyMMdd") + ".xlsx";
        ...
        private void btnCarNoExist_Click(object sender, EventArgs e)
        {
            ...
            if (IsMyCar)
            {
                ...
                MessageBox.Show("\""+ LocalUser.Instance.LogInInformation.ClientName+"\" 차량관리 메뉴에\r\n" +
                    "이미 등록되어 있는 차량입니다." +
                    "\r\n\r\n\"차량번호\"로 조회하시면 됩니다.", "차세로", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return;
            }
        ...
        private void ShareExcelForm_Click(object sender, EventArgs e)
        {
            ...
            fileString = "차량_타사차공유_일괄등록_" + DateTime.Now.ToString("yyyyMMdd") + ".xlsx";
            ieExcel = Properties.Resources.DriverShareUpload;
        ...
        {
            var fileString = "차량_미수금_일괄등록_" + DateTime.Now.ToString("yyyyMMdd") + ".xlsx";
            byte[] ieExcel = Properties.Resources.DriverMisuUpload;
            DirectoryInfo di = new DirectoryInfo(LocalUser.Instance.PersonalOption.DRIVER);
