...
{
    public partial class FrmMN0301_ETC_REFERRAL : Form
    {
        ...
        public FrmMN0301_ETC_REFERRAL(string _mGubun = "", int _mNSTOrderId = 0, string _mCreateTime = "")
        {
        ...
        private void initReferralId()
        {
            ...
            customersNewTableAdapter.FillByClients(clientDataSet.CustomersNew, LocalUser.Instance.LogInInformation.ClientId, 7);
            ...
            var cmbCustomerMIdDataSource = clientDataSet.CustomersNew.Where(c => c.CustomerId == LocalUser.Instance.LogInInformation.CustomerId).Select(c => new { c.SangHo, c.CustomerId }).OrderBy(c => c.SangHo).ToArray();
           ...
           // DCustomer.Add(0, "협력사");
            ...
            {
                cmb_ReferralId.SelectedIndex = 1;
            }
            else
            {
        ...
        private void _Search(int OrderState = 0)
        {
            ...
            using (ShareOrderDataSet ShareOrderDataSet = new ShareOrderDataSet())
            {
                    ...
                    switch (OrderState)
                    {
                        ...
                        case 0:
                            if (_Sort == false)
                            {
                                if (_SortGubun == "ClientId")
                                {
                                    DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderByDescending(c => c.Customer).ToList();
                                ...
                                {
                                    DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderByDescending(c => c.CreateTime).ToList();
                                    //DataSource = DataSource.Where(c => ClientDataSourceList.Contains((int)c.ClientId)).ToList();
                                }
                                else if (_SortGubun == "Start")
                                {
                                    DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderByDescending(c => c.StartName).ToList();
                                    //DataSource = DataSource.Where(c => ClientDataSourceList.Contains((int)c.ClientId)).ToList();
                                }
                                else if (_SortGubun == "Stop")
                                {
                                    DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderByDescending(c => c.StopName).ToList();
                                    //DataSource = DataSource.Where(c => ClientDataSourceList.Contains((int)c.ClientId)).ToList();
                                }

                                else if (_SortGubun == "StartTime")
                                {
                                    DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderByDescending(c => c.StartTime).ToList();
                                    //DataSource = DataSource.Where(c => ClientDataSourceList.Contains((int)c.ClientId)).ToList();
                                }

                                else if (_SortGubun == "StopTime")
                                {
                                    DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderByDescending(c => c.StopTime).ToList();
                                    //DataSource = DataSource.Where(c => ClientDataSourceList.Contains((int)c.ClientId)).ToList();
                                }
                                _Sort = true;
                            ...
                            else
                            {
                                if (_SortGubun == "ClientId")
                                {
                                    DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderBy(c => c.Customer).ToList();
                                    //DataSource = DataSource.Where(c => ClientDataSourceList.Contains((int)c.ClientId)).ToList();
                                }
                                else if (_SortGubun == "Time")
                                {
                                    DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderBy(c => c.CreateTime).ToList();
                                ...
                                {
                                    DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderBy(c => c.StartName).ToList();
                                ...
                                {
                                    DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderBy(c => c.StopName).ToList();
                                ...
                                {
                                    DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderBy(c => c.StartTime).ToList();
                                ...
                                {
                                    DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderBy(c => c.StopTime).ToList();
                ...
                lblSalesPrice.Text = DataSource.Sum(c => c.TradePrice ?? 0).ToString("N0");
                lblAlterPrice.Text = DataSource.Sum(c => c.AlterTradePrice ?? 0).ToString("N0");
                lblClientPrice.Text = DataSource.Sum(c => c.TradePrice + c.AlterTradePrice ?? 0).ToString("N0");
                lblTradePrice.Text = DataSource.Sum(c => c.NTradePrice ?? 0).ToString("N0");
                lblAlterTradePrice.Text = DataSource.Sum(c => c.NAlterTradePrice ?? 0).ToString("N0");
                lblTPrice.Text = DataSource.Sum(c => c.NTradePrice + c.NAlterTradePrice ?? 0).ToString("N0");
            }
        ...
        private void _CustomerSelected()
        {
            ...
            if (_Customer.Fee > 0)
            {
                decimal.TryParse(NTradePrice.Text.Replace(",", ""), out decimal _Price);
                AlterTradePrice.Text = (_Price * 0.01m * _Customer.Fee).ToString("N0");
            }
            CustomerSelectContainer.Visible = false;
        ...
        private void Driver_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else
            {
                if (e.KeyChar == 13)
                {
                    if (LocalUser.Instance.LogInInformation.Client.NeedCustomerId && NTradePrice.Text == "0")
                    {
                        MessageBox.Show("배차를 입력하기 전, 먼저 운송료를 설정하여 주십시오.", "차세로", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                        return;
                    }
        ...
        private void _DriverSelected()
        {
            ...
            if (grid1.CurrentCell != null && DataListSource.Current is Order Current && Current.DriverId == null && Current.TradeId == null && (Current.OrderStatus == 1 || Current.OrderStatus == 2))
            {
               ...
               // Current.SalesPrice = 0;
                ...
                //지급
                Current.NTradePrice = int.Parse(NTradePrice.Text.Replace(",", ""));
                //추가
                Current.NAlterTradePrice = int.Parse(NAlterTradePrice.Text.Replace(",", ""));
                //총지급
                Current.NPrice = Current.NTradePrice.Value + Current.NAlterTradePrice.Value;
        ...
        private void Number_Leave(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else if (Number == NTradePrice || Number == NAlterTradePrice)
                {
                    decimal _TotalPrice = 0;
                    _TotalPrice = (decimal.Parse(NTradePrice.Text.Replace(",", "")) + decimal.Parse(NAlterTradePrice.Text.Replace(",", "")));
                    Number.Text = int.Parse(Number.Text.Replace(",", "")).ToString("N0");

                    NPrice.Text = _TotalPrice.ToString("N0");
                }
        ...
        private void NewOrder_Click(object sender, EventArgs e)
        {
            ...
            var _Customer = CustomerModelList.Find(c => c.CustomerId == 30369);
        ...
        private void InsertOrder_Click(object sender, EventArgs e)
        {
            ...
            if (LocalUser.Instance.LogInInformation.Client.CustomerPay)
            {
                ...
                var _Price = int.Parse(NTradePrice.Text.Replace(",", ""));
                var _Fee = (decimal)(_Price * _Customer.Fee * 0.01m);
                AlterTradePrice.Text = _Fee.ToString("N0");
            ...
            for (int i = 0; i < CarCountValue; i++)
            {
                ...
                //{
                //    nOrder.StartPrice = 0; // int.Parse(Price4.Text.Replace(",", ""));
                //    nOrder.StopPrice = 0; // int.Parse(Price5.Text.Replace(",", ""));
                //    nOrder.DriverPrice = 0; // int.Parse(Price6.Text.Replace(",", ""));
                ...
                //    nOrder.SalesPrice = 0; //int.Parse(Price2.Text.Replace(",", ""));
                ...
                //지급
                nOrder.NTradePrice = int.Parse(NTradePrice.Text.Replace(",", ""));
                //추가
                nOrder.NAlterTradePrice = int.Parse(NAlterTradePrice.Text.Replace(",", ""));
                //총지급
                nOrder.NPrice = nOrder.NTradePrice.Value + nOrder.NAlterTradePrice.Value;

                // nOrder.ClientPrice = nOrder.Price;//nOrder.SalesPrice;
                ...
                //else if (!String.IsNullOrEmpty(StartTimeType4.Text) && StopTimeType4.Text != "0")
                //{
                ...
                if (StopTimeType1.Checked)
                {
                    ...
                    nOrder.StopTime = nOrder.StartTime.Date;
                    if (StopTimeHour1.SelectedIndex > 0 && StopTimeHour2.SelectedIndex > 0)
                    {
                    ...
                    else
                    {
                        ...
                        nOrder.StopTime = Convert.ToDateTime(nOrder.StartTime.AddHours(6).ToString("yyyy-MM-dd HH:mm"));
                ...
                else if (StopTimeType2.Checked)
                {
                    ...
                    if (StopTimeHour1.SelectedIndex > 0 && StopTimeHour2.SelectedIndex > 0)
                    {
                        ...
                        if (StopTimeHalf.Checked)
                        {
                            nOrder.StopTime = _StopTimeType.AddMinutes(30);
                        }
                    }
                    else
                    {
                ...
                else if (StopTimeType3.Checked)
                {
                    ...
                    nOrder.StopTime = _StopTimeType; //MDay;//nOrder.StartTime.Date;
                    if (StopTimeHour1.SelectedIndex > 0 && StopTimeHour2.SelectedIndex > 0)
                    {
                        nOrder.StopTimeHour = StopTimeHour2.SelectedIndex + (StopTimeHour1.SelectedIndex - 1) * 12;
                        nOrder.StopTime = nOrder.StopTime.AddHours(nOrder.StopTimeHour.Value);
        ...
        private void UpdateOrder_Click(object sender, EventArgs e)
        {
            ...
            if (String.IsNullOrEmpty(StartState.Text) || String.IsNullOrEmpty(StopState.Text) || String.IsNullOrEmpty(NTradePrice.Text) || String.IsNullOrEmpty(Item.Text))
            {
                MessageBox.Show("상하차지 운송료/추가정보는 필수 입력사항입니다.", "차세로", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return;
            }
            ...
            using (ShareOrderDataSet ShareOrderDataSet = new ShareOrderDataSet())
            {
               ...
               // uOrder.ClientPrice = 0;
                ...
                //if (PayLocation.SelectedIndex == 2)
                //{
                //    uOrder.StartPrice = 0; // int.Parse(Price4.Text.Replace(",", ""));
                //    uOrder.StopPrice = 0; // int.Parse(Price5.Text.Replace(",", ""));
                //    uOrder.DriverPrice = 0; // int.Parse(Price6.Text.Replace(",", ""));
                ...
                //    uOrder.SalesPrice = 0; // int.Parse(Price2.Text.Replace(",", ""));
                ...
                //지급
                uOrder.NTradePrice = int.Parse(NTradePrice.Text.Replace(",", ""));
                //추가
                uOrder.NAlterTradePrice = int.Parse(NAlterTradePrice.Text.Replace(",", ""));
                //총지급
                uOrder.NPrice = uOrder.NTradePrice.Value + uOrder.NAlterTradePrice.Value;
                //uOrder.ClientPrice = uOrder.SalesPrice;
                ...
                else if (StartTimeType2.Checked)
                {
                    ...
                    else
                    {
                        ...
                        //if (DateTime.Now.Minute < 30)
                        //{
                        //    uOrder.StartTime = Convert.ToDateTime(uOrder.StopTime.ToString("yyyy-MM-dd HH:mm"));
                        //}
                        //else
                        //{
                        //    uOrder.StartTime = Convert.ToDateTime(uOrder.StopTime.AddHours(1).ToString("yyyy-MM-dd HH:mm"));
                        //}
                ...
                else if (StartTimeType3.Checked)
                {
                    ...
                    else
                    {
                        ...
                        uOrder.StartTimeHalf = null;
                        uOrder.StartTimeETC = null;
                        uOrder.StartTime = _StartTimeType.Date.AddHours(8);
                ...
                else if (StopTimeType2.Checked)
                {
                    ...
                    else
                    {
                        uOrder.StopTimeHour = null;
                        uOrder.StopTimeHalf = null;
                        uOrder.StopTime = _StopTimeType.Date.AddHours(8);
                    }
                }
                else if (StopTimeType3.Checked)
                {
                    //var MDay = uOrder.StopTime.Date.AddDays(Convert.ToInt32(DayOfWeek.Monday) - Convert.ToInt32(uOrder.StopTime.DayOfWeek)).AddDays(7);

                    uOrder.StopTimeType = 3;
                    uOrder.StopTime = _StopTimeType;//uOrder.StartTime;
                    if (StopTimeHour1.SelectedIndex > 0 && StopTimeHour2.SelectedIndex > 0)
                    {
                        uOrder.StopTimeHour = StopTimeHour2.SelectedIndex + (StopTimeHour1.SelectedIndex - 1) * 12;
                        uOrder.StopTime = uOrder.StopTime.AddHours(uOrder.StopTimeHour.Value);
                ...
                else if (StopTimeType4.Checked)
                {
                    ...
                    uOrder.StopTime = uOrder.StartTime.AddHours(6);
                }
                else if (!StopTimeType1.Checked && !StopTimeType2.Checked && !StopTimeType3.Checked && !StopTimeType4.Checked)
                {
        ...
        private void DataList_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (e.RowIndex > -1 && e.RowIndex < DataListSource.Count)
            {
                ...
                //추가청구임
                else if (e.ColumnIndex == ColumnAlterTradePrice.Index)
                {
                ...
                //지급운임
                else if (e.ColumnIndex == ColumnNTradePrice.Index)
                {
                    e.Value = (Current.NTradePrice ?? 0).ToString("N0");
                }
                //추가운임
                else if (e.ColumnIndex == ColumnNAlterTradePrice.Index)
                {
                    e.Value = (Current.NAlterTradePrice ?? 0).ToString("N0");
                }
                //총 지급운임
                else if (e.ColumnIndex == ColumnNPrice.Index)
                {
                    e.Value = (Current.NPrice ?? 0).ToString("N0");
                }
        ...
        private void CancelOrder_Click(object sender, EventArgs e)
        {
            ...
            if (DataListSource.Current != null)
            {
                ...
                if (String.IsNullOrEmpty(StartState.Text) || String.IsNullOrEmpty(StopState.Text)
  || String.IsNullOrEmpty(NTradePrice.Text) || String.IsNullOrEmpty(Item.Text))
                {
        ...
        private void DataListSource_CurrentChanged(object sender, EventArgs e)
        {
            ...
            if (DataListSource.Current != null)
            {
                ...
                if (Current.TradeId != null)
                {
                    ...
                    if (Current.OrderStatus == 3)
                    {
                        NTradePrice.Enabled = false;
                    }
                    else
                    {
                        NTradePrice.Enabled = true;
                    }
                }
                ...
                if (Current.MyCarOrder == true)
                {
                    ...
                    if (AlterTradePrice.Enabled == false)
                    {
                        AlterTradePrice.Enabled = true;
                    }
                ...
                if (Current.MyCarOrder == false && Current.OrderStatus == 3 && Current.FOrderId != 0)
                {
                    ...
                    //보관금 수정불가
                    if (AlterTradePrice.Enabled == true)
                    {
                        AlterTradePrice.Enabled = false;
                    }
        ...
        private void ModelToView(Order Current)
        {
            ...
            TotalPrice.Text = (Current.TradePrice + Current.AlterTradePrice ?? 0).ToString("N0");


            NTradePrice.Text = (Current.NTradePrice ?? 0).ToString("N0");
            NAlterTradePrice.Text = (Current.NAlterTradePrice ?? 0).ToString("N0");
            NPrice.Text = (Current.NTradePrice + Current.NAlterTradePrice ?? 0).ToString("N0");
 ...
 || String.IsNullOrEmpty(NTradePrice.Text) || String.IsNullOrEmpty(Item.Text))
        ...
        private void cmb_ReferralId_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //    {
            //        //MessageBox.Show("운송비가 선/착불,선/착불+인수증일 경우 위탁사는 필수 입력사항입니다.", "차세로", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            //        MessageBox.Show("협력사는 필수 입력사항입니다.", "차세로", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            //        cmb_ReferralId.Focus();
            //        return;
            //    }
            //}
        ...
        private void PrevViewOrder(int POrderId)
        {
            ...
            using (ShareOrderDataSet ShareOrderDataSet = new ShareOrderDataSet())
            {
                ...
                if (DataSource.Any())
                {
                    ...
                    NTradePrice.Text = (DataSource.First().TradePrice ?? 0).ToString("N0");
                    //Price2.Text = (DataSource.First().SalesPrice ?? 0).ToString("N0");
                    AlterTradePrice.Text = (DataSource.First().AlterPrice ?? 0).ToString("N0");
                   ...
                   // nOrder.CustomerTeam = (int)cmbCustomerTeam.SelectedValue;
                    ...
                    else if (StopTimeType3.Checked)
                    {
                      //  var MDay = DateTime.Now.Date.AddDays(Convert.ToInt32(DayOfWeek.Monday) - Convert.ToInt32(DateTime.Today.DayOfWeek)).AddDays(7);
                        ...
                        nOrder.StopTimeType = 3;
                        nOrder.StopTime = _StopTimeType;// nOrder.StartTime.Date;
                        if (StopTimeHour1.SelectedIndex > 0 && StopTimeHour2.SelectedIndex > 0)
                        {
                            nOrder.StopTimeHour = StopTimeHour2.SelectedIndex + (StopTimeHour1.SelectedIndex - 1) * 12;
                            nOrder.StopTime = nOrder.StopTime.AddHours(nOrder.StopTimeHour.Value);
        ...
        private void addressRotate_Click(object sender, EventArgs e)
        {
            ...
            var _StopTimeType5 = dtpStartTime.Value;
            if (StopTimeType1.Checked)
            {
                _StopTimeType1 = true;
            }
        ...
        private void CallPass24OldToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                //운송료
                SendTextToAnotherApplication.SetTextbyTwNumEdit(2, NTradePrice.Text.Replace(",", ""));
                SendTextToAnotherApplication.SetTextbyTwNumEdit(1, "0");
                SendTextToAnotherApplication.SetTextbyTwNumEdit(0, NTradePrice.Text.Replace(",", ""));
            }
            //선착불
        ...
        private void CallPass24NewToolStripMenuItem_Click(object sender, EventArgs e)
        {
           ...
           // SendTextToAnotherApplication.SetText(5, StartTimeType4.Text);
            ...
            if (PayLocation.SelectedIndex == 1)
            {
                ...
                //합계
                SendTextToAnotherApplication.SetTextbyTwNumEdit(1, NTradePrice.Text.Replace(",", ""));
