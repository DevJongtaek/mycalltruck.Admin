...
{
    public partial class FrmMN0301_ETCCustomer : Form
    {
        ...
        public FrmMN0301_ETCCustomer(string _mGubun = "", int _mNSTOrderId = 0, string _mCreateTime = "")
        {
        ...
        private void _Search(int OrderState = 0)
        {
            ...
            using (ShareOrderDataSet ShareOrderDataSet = new ShareOrderDataSet())
            {
                ...
                switch (OrderState)
                {
                    ...
                    case 0:
                        if (_Sort == false)
                        {
                            ...
                            {
                                DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderByDescending(c => c.ReferralName).ToList();
                                //DataSource = DataSource.Where(c => ClientDataSourceList.Contains((int)c.ClientId)).ToList();
                            }
                            _Sort = true;
                        ...
                        else
                        {
                            ...
                            {
                                DataSource = ShareOrderDataSet.Orders.Include("DriverModel").Include("TradeModel").Where(c => ClientDataSourceList.Contains((int)c.ClientId)).OrderBy(c => c.ReferralName).ToList();
                ...
                lblCount.Text = DataSource.Where(C => C.CustomerId == LocalUser.Instance.LogInInformation.CustomerId).ToList().Count.ToString("N0");
                lblPrice1.Text = DataSource.Where(C => C.CustomerId == LocalUser.Instance.LogInInformation.CustomerId).ToList().Sum(c => c.SalesPrice ?? 0).ToString("N0");
                lblPrice3.Text = DataSource.Where(C => C.CustomerId == LocalUser.Instance.LogInInformation.CustomerId).ToList().Sum(c => c.AlterPrice ?? 0).ToString("N0");
                lblTotalPrice.Text = DataSource.Where(C => C.CustomerId == LocalUser.Instance.LogInInformation.CustomerId).ToList().Sum(c => c.SalesPrice + c.AlterPrice ?? 0).ToString("N0");


                if (LocalUser.Instance.LogInInformation.CustomerUserId != 0)
                {
                    DataListSource.DataSource = DataSource.Where(C => C.CustomerTeam == LocalUser.Instance.LogInInformation.CustomerTeamId).ToList();
                    lblCount.Text = DataSource.Where(C => C.CustomerTeam == LocalUser.Instance.LogInInformation.CustomerTeamId).ToList().Count.ToString("N0");
                    lblPrice1.Text = DataSource.Where(C => C.CustomerTeam == LocalUser.Instance.LogInInformation.CustomerTeamId).ToList().Sum(c => c.SalesPrice ?? 0).ToString("N0");
                    lblPrice3.Text = DataSource.Where(C => C.CustomerTeam == LocalUser.Instance.LogInInformation.CustomerTeamId).ToList().Sum(c => c.AlterPrice ?? 0).ToString("N0");
                    lblTotalPrice.Text = DataSource.Where(C => C.CustomerTeam == LocalUser.Instance.LogInInformation.CustomerTeamId).ToList().Sum(c => c.SalesPrice + c.AlterPrice ?? 0).ToString("N0");
                }
                else
                {
                    if (cmbTeam.SelectedIndex > 0)
                    {
                        ...
                        lblCount.Text = DataSource.Where(C => C.CustomerTeam == (int)cmbTeam.SelectedValue).ToList().Count.ToString("N0");
                        lblPrice1.Text = DataSource.Where(C => C.CustomerTeam == (int)cmbTeam.SelectedValue).ToList().Sum(c => c.SalesPrice ?? 0).ToString("N0");
                        lblPrice3.Text = DataSource.Where(C => C.CustomerTeam == (int)cmbTeam.SelectedValue).ToList().Sum(c => c.AlterPrice ?? 0).ToString("N0");
                        lblTotalPrice.Text = DataSource.Where(C => C.CustomerTeam == (int)cmbTeam.SelectedValue).ToList().Sum(c => c.SalesPrice + c.AlterPrice ?? 0).ToString("N0");
                    }
        ...
        private void SetMouseClick(Control mControl)
        {
            foreach (Control nControl in mControl.Controls)
            {
                if (nControl == StartSelect || nControl == StopSelect
                    || nControl == CustomerSelectContainer )
                {
                    continue;
                }
        ...
        private void Number_Leave(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else if (Number == Price1 || Number == Price3)
                {
                    decimal _TotalPrice = 0;
                    _TotalPrice = (decimal.Parse(Price1.Text.Replace(",", "")) + decimal.Parse(Price3.Text.Replace(",", "")));
  ...
  || String.IsNullOrEmpty(Price1.Text) || String.IsNullOrEmpty(Item.Text))
        ...
        private void ModelToView(Order Current)
        {
            ...
            Price1.Text = (Current.SalesPrice ?? 0).ToString("N0");
            ...
            var _Total = Current.SalesPrice + Current.AlterPrice;
            ...
            if (Query.Any())
            {
                ...
                else
                {
                    ...
                    if(cmb_ReferralId.SelectedIndex == 0)
                    {
                        ...
                        TotalPrice.Text = (Current.SalesPrice ?? 0).ToString("N0");
                    }
            ...
            else
            {
             ...
             || String.IsNullOrEmpty(Item.Text))
                ...
                for (int i = 0; i < CarCountValue; i++)
                {
                    ...
                    //}
                    nOrder.TradePrice = int.Parse(Price1.Text.Replace(",", ""));
                    nOrder.SalesPrice = nOrder.TradePrice; // int.Parse(Price2.Text.Replace(",", ""));
                    nOrder.AlterPrice = int.Parse(Price3.Text.Replace(",", ""));
                    nOrder.Price = nOrder.TradePrice.Value + nOrder.AlterPrice.Value;
                    ...
                    else if (StartTimeType3.Checked)
                    {
                        ...
                        nOrder.StartDate = DateTime.Now.AddDays(1).Date;
                        ...
                        {
                            //내일오전8시
                            nOrder.StartTime = DateTime.Now.AddDays(1).Date.AddHours(8);
                        }
                        StartTime = "내상/";
                    }
                    else if (!StartTimeType1.Checked && !StartTimeType2.Checked && !StartTimeType3.Checked)
                    ...
                    else if (StopTimeType2.Checked)
                    {
                        ...
                        nOrder.StopTime = nOrder.StartTime.AddDays(1).Date;
                        if (StopTimeHour1.SelectedIndex > 0 && StopTimeHour2.SelectedIndex > 0)
                        {
                            nOrder.StopTimeHour = StopTimeHour2.SelectedIndex + (StopTimeHour1.SelectedIndex - 1) * 12;
                            nOrder.StopTime = nOrder.StopTime.AddHours(nOrder.StopTimeHour.Value);
                        ...
                        {
                            nOrder.StopTime = nOrder.StartTime.AddDays(1).Date.AddHours(8);
                        }
                        StopTime = "내착";
                    }
                    else if (StopTimeType3.Checked)
